...
namespace ManoTests\GeneratorPlus;
...
use ManoTests\GeneratorPlus\Data\TestEvent;
...
class GeneratorPlusTest extends TestCase
{
	...
	{
		$generator = GeneratorPlus::createFromCallable(function (GeneratorEventDispatcher $kastan) {
			yield 1;
			$kastan->dispatch(new TestEvent(123));
			yield 2;
			$kastan->dispatch(new class () implements GeneratorPlusEvent {
			});
			yield 3;
		});
		...
		$items = [];
		$generator->attachEvent(TestEvent::class, function (TestEvent $event) use (&$items) {
			$items[] = $event->id;
		});
		...
		$this->assertSame([1, 123, 2, 3], $items);
	}
